 ...
 *   1. adsb.lol — aircraft state vectors (keyless, ODbL 1.0)
...
/**
 * User-facing coverage caveat, surfaced in the layer panel via X-Flight-Coverage.
 * ASCII ONLY: this travels as an HTTP header value and Node rejects non-ASCII
 * characters there (an em dash here returned 502 on every flight poll).
 */
const FLIGHT_COVERAGE_NOTE = 'regional coverage - not all aircraft';
...
const RADIO_USER_AGENT = 'EyeOfAtlas/1.0 (Radio Browser directory client)';
...
function celestrakProxy() {
  ...
  async function fetchUpstream(group) {
    ...
    const res = await fetch(url.toString(), {
      ...
      headers: { 'User-Agent': 'eye-of-atlas-celestrak-proxy/1.0 (+https://github.com/DNRosacena/eye-of-atlas)' },
    });
    ...
    try {
      const upstream = await fetch(endpoint, {
        ...
        headers: {
          ...
          'User-Agent': 'eye-of-atlas-overpass-proxy/1.0',
        },
        body,
        signal: controller.signal,
      });
        ...
        try {
          ...
          try {
            const upstreamRes = await fetch(upstream, {
              signal: controller.signal,
              headers: { 'User-Agent': 'eye-of-atlas/dev (local)' },
            });
    ...
    try {
      const upstream = await fetch(
        `https://api.adsb.lol/v2/lat/${roundedLat}/lon/${roundedLon}/dist/${ADSBLOL_POINT_RADIUS_NM}`,
        {
          headers: {
            Accept: 'application/json',
            'User-Agent': 'eye-of-atlas-adsblol-regional-fallback/1.0',
          },
          signal: controller.signal,
        },
      );
...
/**
 * Vite plugin: live-flight proxy backed by adsb.lol.
 *
 * Replaces the former OpenSky proxy. OpenSky's licence is non-commercial and
 * explicitly prohibits "advertisements on web pages/applications using the
 * API", which is exactly this product's model — see COMMERCIAL_COMPLIANCE.md
 * §3.4. adsb.lol publishes its database under ODbL 1.0, which permits
 * commercial use with attribution.
 *
 * The response shape is unchanged: adsbLolFallback.js normalises adsb.lol
 * records into the OpenSky state-vector shape the Flights renderer already
 * consumes, so no client rendering code changed with this swap.
 *
 * COVERAGE IS REGIONAL, NOT GLOBAL. adsb.lol is a community ADS-B network:
 * dense over Europe and North America, sparse elsewhere. Each response carries
 * X-Flight-Coverage, which the client surfaces in the layer panel so the
 * limitation is visible to users rather than read as missing aircraft.
 *
 * Requires lat/lon: this is a bounded radius query, not a global snapshot.
 *
 * @returns {import('vite').Plugin}
 */
function flightsProxy() {
  const install = (server) => {
    server.middlewares.use('/api/flights', async (req, res) => {
      try {
        ...
        const snapshot = await fetchAdsbLolPointFallback(req);
        if (!snapshot) {
          // No upstream and no cache. Fail soft with an empty, well-formed
          // snapshot so the layer renders "no contacts" rather than erroring.
          res.writeHead(503, {
            'Content-Type': 'application/json',
            'Cache-Control': 'no-store',
            'X-Flight-Source': 'adsb.lol',
            'X-Flight-Coverage': FLIGHT_COVERAGE_NOTE,
            'Retry-After': '30',
          });
        ...
        res.writeHead(200, {
          'Content-Type': 'application/json',
          'Cache-Control': 'no-store',
          'X-Flight-Cache': snapshot.cacheStatus,
          'X-Flight-Source': 'adsb.lol',
          'X-Flight-Coverage': FLIGHT_COVERAGE_NOTE,
          'X-Flight-Count': String(snapshot.count),
        });
        res.end(snapshot.body);
      } catch (error) {
        console.warn('[flights-proxy]', error?.message || error);
        if (res.headersSent) return;
        res.writeHead(502, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'flight feed unavailable' }));
      }
    });
  };
  return {
    name: 'flights-proxy',
    configureServer: install,
    // Registered for preview too: the former OpenSky proxy was dev-only, which
    // meant `vite preview` silently served index.html for this route
    // (docs/GEV-INSPECTION.md §5.4).
    configurePreviewServer: install,
  };
}
        ...
        try {
          ...
          try {
            upstream = await fetch(upstreamUrl.toString(), {
              ...
              headers: {
                Accept: 'application/json',
                'User-Agent': 'eye-of-atlas-gbfs-proxy/1.0',
              },
              signal: controller.signal,
            });
          } finally {
...
export async function fetchCctvImageFromUpstream(url, {
  ...
  try {
    const upstream = await fetchImpl(url, {
      headers: { 'User-Agent': 'eye-of-atlas-cctv-proxy/1.0' },
      signal: controller.signal,
    });
      ...
      server.middlewares.use('/api/cctv', async (req, res) => {
        try {
          ...
          if (url.pathname.startsWith('/media/')) {
            ...
            try {
              const upstreamHeaders = { 'User-Agent': 'eye-of-atlas-cctv-proxy/1.0' };
      ...
      server.middlewares.use('/api/adsblol/mil', async (req, res) => {
        try {
          ...
          const upstream = await fetch('https://api.adsb.lol/v2/mil', {
            headers: { 'User-Agent': 'eye-of-atlas-adsblol-proxy/1.0' },
          });
          const body = await upstream.text();
          if (upstream.ok) {
            _cache = body;
 ...
 *
 * The OpenSky /tracks/all backfill was removed here: OpenSky's licence is
 * non-commercial (COMMERCIAL_COMPLIANCE.md §3.4). Civil-flight trails are now
 * built from locally accumulated fixes only.
 ...
 * Replaces the browser's direct call to Google Geocoding, which needed the
 * client-exposed Google key. The owner dropped that key (no card on file), so
 * search — a P0 MVP feature — would otherwise throw.
 *
 * Nominatim's usage policy is strict and non-negotiable:
 *   - an absolute maximum of 1 request per second       -> shares the existing
 *     _nominatimQueue with the reverse-geocode path, so BOTH together stay
 *     inside one budget rather than one each
 *   - "Results must be cached on your side"             -> GEOCODE_CACHE below
 *   - a User-Agent that identifies the application      -> set on the request
 *   - applications whose PRIMARY function is geocoding must self-host. We are a
 *     globe that happens to have a search box, so we are on the right side of
 *     that line — but it is a line, and heavy use means self-hosting or an
 *     alternative provider (COMMERCIAL_COMPLIANCE.md §6.4).
 *
 * The response is normalised into the Google Geocoding SHAPE the client already
 * consumes, so src/locations.js keeps its framing logic unchanged — the same
 * trick adsbLolFallback.js uses for flights.
 */
...
function geocodeProxy() {
  const install = (middlewares) => {
    middlewares.use('/api/geocode', async (req, res) => {
      try {
        const incoming = new URL(req.url || '', 'http://localhost');
        const query = String(incoming.searchParams.get('q') || '').trim().slice(0, 200);
        if (!query) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
        ...
        const body = JSON.stringify(nominatimToGoogleGeocode(await task));
        GEOCODE_CACHE.delete(cacheKey);
        GEOCODE_CACHE.set(cacheKey, { at: Date.now(), body });
        while (GEOCODE_CACHE.size > GEOCODE_CACHE_MAX) {
          GEOCODE_CACHE.delete(GEOCODE_CACHE.keys().next().value);
        }
        res.writeHead(200, { 'Content-Type': 'application/json', 'X-Geocode-Cache': 'MISS' });
        res.end(body);
      } catch (error) {
        console.warn('[geocode-proxy]', error?.message || error);
        if (res.headersSent) return;
        res.writeHead(502, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ status: 'UNKNOWN_ERROR', results: [] }));
      }
    });
  };
  return {
...
async function fetchRegionalNews(place) {
  ...
  try {
    const payload = await fetchRegionalJson(`https://api.gdeltproject.org/api/v2/doc/doc?${params}`, {
      headers: { 'User-Agent': 'EyeOfAtlas/0.1' },
      timeoutMs: 12_000,
    });
    const articles = normalizeRegionalArticles(payload, 5);
    return { status: articles.length ? 'ready' : 'empty', query, articles, source: 'GDELT Project' };
  } catch {
...
function regionalBriefProxy() {
  async function refresh(point, key) {
    // Weather removed: Open-Meteo's free API forbids ad-supported use
    // (COMMERCIAL_COMPLIANCE.md §2.1). The brief keeps place + headlines. The
    // `weather` field is retained as null so the payload shape — and every
    // consumer's null-handling — stays unchanged.
    const placeResult = await Promise.allSettled([fetchRegionalPlace(point)]).then((r) => r[0]);
