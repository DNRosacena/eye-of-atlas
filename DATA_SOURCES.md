...
- **Eye of Atlas ships only commercially usable data.** Upstream God's Eye View bundled some non-permissive data (notably TeleGeography, CC BY-NC-SA) behind a licence carve-out. This fork has REMOVED every source whose terms forbid use in a monetised product — see `COMMERCIAL_COMPLIANCE.md`.
- **If your use doesn't fit a dataset's license, remove that dataset.** Removed here for that reason: TeleGeography submarine cables (CC BY-NC-SA), OpenSky (non-commercial, prohibits ad-supported use), Google News RSS (personal use only), Open-Meteo's free API (forbids ad-supported use).
...
| **adsb.lol point API** | **Primary** live-flight snapshot (bounded radius around the view) | ODbL 1.0 | adsb.lol contributors; `api.adsb.lol/v2/lat/{lat}/lon/{lon}/dist/{radius}` |
...
| **Esri World Imagery** (ArcGIS Online tile service) | The keyless satellite basemap — the default landing when no Google/ion credential is configured, and the "Esri Satellite" map stack | ⚠️ **NOT LICENSED FOR COMMERCIAL USE AS CURRENTLY CALLED.** The keyless `services.arcgisonline.com` endpoint is governed by the [Esri Master License Agreement](https://www.esri.com/en-us/legal/terms/full-master-agreement) via the [Esri Terms of Use](https://www.esri.com/en-us/legal/terms/web-site-service) §2.2, which defines Noncommercial Use as provided at no charge *and* generating no income. An ad-supported deployment needs [ArcGIS Location Platform](https://location.arcgis.com/) (API key; free tier 2M basemap tiles/month; commercial deployment licence). See `COMMERCIAL_COMPLIANCE.md` §6.6 | "Powered by Esri — Source: Esri, Vantor, Earthstar Geographics, and the GIS User Community" (must match the service's own declared attribution; Maxar rebranded to Vantor) |
...
| **GDELT Project DOC 2.0** | Location-matched cockpit headlines (sole source) | [GDELT Terms of Use](https://www.gdeltproject.org/about.html#termsofuse): unrestricted academic/commercial/governmental dataset use, with citation and link required; linked articles retain publisher terms | "GDELT Project" plus each article's linked publisher/domain |
...
- **adsb.lol flights (primary).** The server requests a cached, capped 250 nm adsb.lol point snapshot around the current camera subpoint. This is **regional observed context, not worldwide completeness** — coverage is dense over Europe and North America and sparse elsewhere. That caveat is surfaced in the Flights layer row (`regional coverage - not all aircraft`) and in the Data attribution popover, because users must not read partial coverage as complete coverage. Military ICAOs remain reconciled through the dedicated military registry rather than duplicated.
...
- **Cockpit regional briefing.** `/api/regional-brief` rounds aircraft coordinates into 0.1° cache cells, caches results for five minutes, and serializes Nominatim calls at no more than one request per second. Headlines come from **GDELT DOC 2.0 only** — GDELT permits academic, commercial and governmental use with a citation and link. (Google News RSS was removed: its terms restrict use to personal, noncommercial purposes and explicitly forbid using it to drive traffic for advertising.) The Data attribution popover identifies the headline source; article links retain publisher attribution. Headlines are location-query matches, not verified incidents, risk rankings, or evidence that a location is safe. Empty, partial, stale, and unavailable source states remain distinct. **Weather was removed** from the briefing: Open-Meteo's free API forbids ad-supported use.
...
The required Google Maps / Cesium credit renders on the on-globe credit line (`#cesium-credits`, bottom-left) and must stay visible — including in clean-view and recording modes (the whole line, logo + "Google Maps" + the "Data attribution" link, stays on screen; only the GEV panels/HUD fade). The layer-specific credits (adsb.lol, OSM datacenters/dams/roads, NASA FIRMS, CelesTrak, USGS, GDELT, City of Austin, GBFS, Radio Browser, AISStream) are registered into the expandable **"Data attribution"** popover on that credit line via `viewer.creditDisplay.addStaticCredit(new Cesium.Credit(html, /* showOnScreen */ false))` — see `src/data/dataCredits.js`. When you add a new data source, add its license and attribution to this file **and** append an entry to `DATA_CREDITS` in `src/data/dataCredits.js` so it surfaces in the app.
