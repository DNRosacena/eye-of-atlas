...
| **Esri World Imagery** (via [ArcGIS Location Platform](https://location.arcgis.com/)) | Satellite basemap and the "Esri Satellite" map stack; the keyless landing when no Google/ion credential is configured | [ArcGIS Location Platform Agreement](https://www.esri.com/en-us/legal/requirements/arcgis-location-platform-agreement) — carries a **commercial deployment licence**; free tier 2M basemap tiles/month; **API key required** (`ARCGIS_API_KEY`). Attribution mandatory (§3.1(d)(4)); tiles must not be cached beyond their caching headers (§3.1(d)(6)). The keyless `services.arcgisonline.com` endpoint this replaced is Noncommercial-only — do not revert to it. See `COMMERCIAL_COMPLIANCE.md` §6.6 | "Powered by Esri — Source: Esri, Vantor, Earthstar Geographics, and the GIS User Community" |
